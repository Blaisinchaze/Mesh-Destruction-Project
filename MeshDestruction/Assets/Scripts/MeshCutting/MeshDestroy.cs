...
public class MeshDestroy : MonoBehaviour
{
    ...
    public void DestroyWithPlane(Plane plane)
    {
        useCutPlane = true;
        cutPlane = plane;
        DestroyMesh();
    ...
    private void DestroyMesh()
    {
        ...
        if(useCutPlane)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                var plane = cutPlane;
                //new Plane(UnityEngine.Random.onUnitSphere, new Vector3(cutPlane.transform.position.x,
                //                                                                    cutPlane.transform.position.y,
                //                                                                    cutPlane.transform.position.z));

                //plane = new Plane(cutPlane.GetComponent<Mesh>().vertices[0], cutPlane.GetComponent<Mesh>().vertices[1], cutPlane.GetComponent<Mesh>().vertices[2]);
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].MakeGameobject(this,player);
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original, Collider player)
        {
            ...
            var fadeAway = GameObject.AddComponent<FadeAway>();
